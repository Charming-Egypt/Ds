// ==================== PAYMENT & BOOKING ====================
function paymentMethodsBlock(currentMethod, onchangeFn, allowCash) {
  const methods = [
    {
      id: 'card',
      label: 'Card or E-Wallet',
      sub: 'Visa, Mastercard, Amex & digital wallets',
      logos: `<span class="pay-logo-row">
                <i class="fa-brands fa-cc-visa" title="Visa"></i>
                <i class="fa-brands fa-cc-mastercard" title="Mastercard"></i>
                <i class="fa-brands fa-cc-amex" title="Amex"></i>
                <span class="pay-logo-wallet"><i class="fa-solid fa-wallet"></i></span>
              </span>`
    },
    {
      id: 'instapay',
      label: 'InstaPay',
      sub: 'Instant transfer from your bank app',
      logos: `<span class="pay-logo-instapay"><i class="fa-solid fa-bolt"></i>InstaPay</span>`
    },
  ];
  if (allowCash) {
    methods.push({
      ...
      sub: 'Pay at the hotel reception',
      logos: `<span class="pay-logo-cash"><i class="fa-solid fa-money-bill-wave"></i></span>`
    });
  }
  return methods.map(m => `
    <label class="pay-method-option ${currentMethod === m.id ? 'selected' : ''}">
      <input type="radio" name="paymethod" value="${m.id}" ${currentMethod === m.id ? 'checked' : ''} onchange="${onchangeFn}('${m.id}')" class="pay-method-radio">
      <span class="pay-method-check"><i class="fa-solid fa-check"></i></span>
      <span class="pay-method-info">
        <span class="pay-method-label">${m.label}</span>
        <span class="pay-method-sub">${m.sub}</span>
      </span>
      <span class="pay-method-logos">${m.logos}</span>
    </label>`).join('');
}
  ...
  if (step === 2) {
    ...
    bodyHtml = `
      <div class="p-5">
        ...
        <div class="space-y-3 mb-5">${paymentMethodsBlock(state.bookingDraft.payment, 'setHotelPaymentMethod', true)}</div>
...
function renderExcursionBookingStep(step) {
  ...
  if (step === 2) {
    ...
    bodyHtml = `
      <div class="p-5">
        <h3 class="font-display text-lg font-bold mb-3">Payment Method</h3>
        <div class="space-y-3 mb-5">${paymentMethodsBlock(state.bookingDraft.payment, 'setExcursionPaymentMethod', false)}</div>
        <div class="card rounded-2xl p-4 space-y-2 mb-6">
...
function renderTransferBookingStep(step) {
  ...
  if (step === 2) {
    ...
    bodyHtml = `
      <div class="p-5">
        <h3 class="font-display text-lg font-bold mb-3">Payment Method</h3>
        <div class="space-y-3 mb-5">${paymentMethodsBlock(state.bookingDraft.payment, 'setTransferPaymentMethod', false)}</div>
        <div class="card rounded-2xl p-4 space-y-2 mb-6">
...
function setTransferDirection(dir) {
  ...
  if (!a || !d) return;
  a.style.background = dir === 'Airport to Hotel' ? 'linear-gradient(135deg,#e0925a,#7a3f1c)' : 'transparent';
  a.style.color = dir === 'Airport to Hotel' ? '#fff' : 'var(--text-secondary)';
  d.style.background = dir === 'Hotel to Airport' ? 'linear-gradient(135deg,#e0925a,#7a3f1c)' : 'transparent';
...
function bookingDetailBody(b) {
  const paymentLabel = b.payment === 'cash' ? 'Cash on Arrival' : b.payment === 'instapay' ? 'InstaPay' : 'Card / E-Wallet';
