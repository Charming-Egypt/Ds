...
function renderBookingConfirmation(b) {
  ...
  const page = document.createElement('div'); page.id = 'bookingConfirmPage'; page.className = 'page';
  page.innerHTML = `<div class="min-h-screen dark-scene relative overflow-hidden pb-10"><div class="stars-container" id="confirmStars"></div><div class="relative z-10 px-5 pt-6"><div class="text-center mb-8"><div class="w-24 h-24 mx-auto mb-5 relative"><div class="absolute inset-0 bg-gold-400/20 rounded-full animate-ping"></div><div class="relative w-full h-full rounded-full bg-gradient-to-br from-violet-400 to-violet-700 flex items-center justify-center shadow-2xl"><i class="fa-solid fa-check text-4xl text-white"></i></div></div><h2 class="font-display text-2xl font-bold text-white mb-1">Booking Confirmed!</h2><p class="text-white/60 text-sm">${b.paymentStatus === 'pending_cash' ? 'Pay cash on arrival.' : 'Payment received.'}</p></div></div><div class="relative z-10 rounded-t-[28px] mt-4 p-5" style="background:var(--bg-card)"><div class="card rounded-2xl p-3 flex gap-3 mb-4"><img src="${b.image}" class="w-16 h-16 rounded-xl object-cover"><div><h3 class="font-display font-bold">${b.hotelName || b.title || b.vehicleType}</h3><p class="text-[10px]">${b.roomType || b.category || ''}</p></div></div><div class="flex justify-between mb-2"><span>Booking ID</span><span class="font-bold">${b.id}</span></div><div class="flex justify-between mb-2"><span>Check-in</span><span>${utils.formatDate(b.checkin || b.date)}</span></div><div class="flex justify-between mb-4"><span>Total</span><span class="font-bold text-violet-500">${b.priceFormatted}</span></div><button onclick="finishBooking('bookings')" class="btn-violet w-full py-4 rounded-2xl font-bold mb-3">View My Bookings</button><button onclick="finishBooking('home')" class="btn-outline-violet w-full py-4 rounded-2xl font-bold">Back to Home</button></div></div>`;
  document.getElementById('mainApp').appendChild(page); page.classList.add('active'); window.scrollTo(0,0);
  utils.createStars('confirmStars'); utils.confetti();
}
...
async function loadReviews(type, id, listElId, summaryElId) {
  try {
    ...
    if (listEl) {
      listEl.innerHTML = reviews.length ? reviews.slice().reverse().map(r => `<div class="field-box rounded-xl p-3"><div class="flex items-center justify-between mb-1"><div class="flex items-center gap-2"><div class="w-8 h-8 bg-violet-50 rounded-full flex items-center justify-center font-bold">${(r.name || '?').charAt(0)}</div><div><p class="text-xs font-semibold">${r.name}</p><div class="flex">${utils.renderStars(r.rating)}</div></div></div><span class="text-[9px]">${utils.formatDate(r.createdAt)}</span></div><p class="text-xs">${r.comment}</p></div>`).join('') : '<p class="text-xs text-center py-4">No reviews yet</p>';
    }
    if (summaryElId) { const avg = utils.avgRating(reviews); const el = document.getElementById(summaryElId); if (el) el.textContent = avg ? avg.toFixed(1) : '—'; }
  } catch (e) { console.warn('Failed to load reviews', e); }
}
function openReviewModal(type, id) { state.reviewTarget = { type, id }; document.getElementById('reviewBookingId').value = ''; document.getElementById('reviewName').value = currentUser?.displayName || ''; document.getElementById('reviewComment').value = ''; const rating = document.getElementById('reviewRating'); if (rating) rating.value = 5; document.getElementById('reviewModal').classList.remove('hidden'); }
async function submitReview(e) { e.preventDefault(); const bookingId = document.getElementById('reviewBookingId').value.trim().toUpperCase(); const name = document.getElementById('reviewName').value.trim(); const comment = document.getElementById('reviewComment').value.trim(); const rating = document.getElementById('reviewRating')?.value || 5; if (!bookingId || !comment) return toast('Booking ID and comment required', 'error'); try { await apiFetch('/api/reviews', { method: 'POST', body: JSON.stringify({ type: state.reviewTarget.type, id: state.reviewTarget.id, bookingId, name, comment, rating }) }); document.getElementById('reviewModal').classList.add('hidden'); toast('Review submitted!', 'success'); loadReviews(state.reviewTarget.type, state.reviewTarget.id, state.reviewTarget.type === 'hotel' ? 'hotelReviewsList' : 'excursionReviewsList', null); } catch (e) { toast(e.message, 'error'); } }
