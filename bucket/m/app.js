...
const API = {
  ...
  // ====== NEW: قراءة الملفات من الـ Worker الجديد ======
  async readCatalogFile(fileName) {
    const res = await fetch(`${CONTENT_WORKER_URL}/file?file=${encodeURIComponent(fileName)}`);
    const data = await res.json();
    if (!res.ok || data.error) throw new Error(data.error || 'Failed to read ' + fileName);
...
const utils = {
  ...
  toLocalIso(d) { const y = d.getFullYear(), m = String(d.getMonth() + 1).padStart(2, '0'), day = String(d.getDate()).padStart(2, '0'); return `${y}-${m}-${day}`; },
...
const sidebar = {
  open() { document.getElementById('sideDrawer').classList.add('open'); document.getElementById('sideDrawerOverlay').classList.add('open'); },
  close() { document.getElementById('sideDrawer').classList.remove('open'); document.getElementById('sideDrawerOverlay').classList.remove('open'); }
};
...
const kashier = {
  ...
  showModal(innerHtml, onCancel) {
    ...
    if (!modal) { modal = document.createElement('div'); modal.id = 'kashierModal'; modal.className = 'modal-overlay'; document.body.appendChild(modal); }
...
function saveGlobalBooking(booking) {
  if (fbEnabled) { fbDB.ref('allBookings/' + booking.id).set(booking).catch(e => console.warn('Could not save allBookings record', e)); } else { const map = JSON.parse(localStorage.getItem('ds_all_bookings') || '{}'); map[booking.id] = booking; localStorage.setItem('ds_all_bookings', JSON.stringify(map)); }
}
...
function markBookingReviewedGlobal(bookingId, ownerUid) {
  if (fbEnabled) {
    ...
    const map = JSON.parse(localStorage.getItem('ds_all_bookings') || '{}');
    if (map[bookingId]) { map[bookingId].reviewed = true; localStorage.setItem('ds_all_bookings', JSON.stringify(map)); }
    const idx = state.bookings.findIndex(b => b.id === bookingId);
    if (idx > -1) { state.bookings[idx].reviewed = true; utils.save(); bookings.render(); ui.updateProfileStats(); }
  }
}
...
const reviews = {
  ...
  detachListeners() { Object.keys(this.liveRefs).forEach(k => { this.liveRefs[k].ref.off('value', this.liveRefs[k].cb); }); this.liveRefs = {}; }
};
...
function closeHotelPage() { reviews.detachListeners(); const p = document.getElementById('hotelDetailPage'); if (p) p.remove(); nav.go('hotels'); }
...
function startBooking(hotelId, roomIndex) {
  ...
  const required = Math.ceil((state.guests.adults + state.guests.children) / maxOcc);
  if (required > state.guests.rooms) { state.guests.rooms = required; utils.toast('Room count adjusted to fit this room type', 'info'); }
  ...
  state.bookingDraft = { name: 'Ahmed Mohamed', email: '', phone: '', requests: '', payment: 'card', checkin: ci || utils.addDays(utils.todayIso(), 1), checkout: co || utils.addDays(utils.todayIso(), 3) };
  renderBookingStep(2);
}
...
function renderBookingStep(step) {
  ...
  if (existing) { reviews.detachListeners(); existing.remove(); }
...
function persistBooking(b) {
  saveGlobalBooking(b);
  if (fbEnabled && currentUser) { fbDB.ref('users/' + currentUser.uid + '/bookings/' + b.id).set(b); } else { state.bookings.unshift(b); utils.save(); }
...
function payAndConfirmHotelBooking(roomTotal, taxes, total, nights) {
  ...
  kashier.pay({
    ...
    onSuccess: (result) => {
      ...
      const b = {
        id: orderId, type: 'hotel', hotelId: h.id, hotelName: h.name, image: h.image, location: h.location, rating: h.rating,
        name: state.bookingDraft.name, email: state.bookingDraft.email, phone: state.bookingDraft.phone, requests: state.bookingDraft.requests || '',
        checkin: state.bookingDraft.checkin, checkout: state.bookingDraft.checkout, guests, rooms: state.guests.rooms, roomType: r.type, nights,
        payment: state.bookingDraft.payment, paymentStatus: result.status, transactionRef: result.transactionRef, priceUsd: total, priceFormatted: utils.formatPrice(total),
        status: 'upcoming', reviewed: false, createdAt: new Date().toISOString(), uid: (fbEnabled && currentUser) ? currentUser.uid : null
      };
      persistBooking(b);
      renderBookingConfirmation(b);
    },
    onCancel: () => { btn.disabled = false; btn.innerHTML = orig; }
  });
  btn.disabled = false;
  btn.innerHTML = orig;
}
...
function finishBooking(target) {
  ...
  if (hp) { reviews.detachListeners(); hp.remove(); }
  const ep = document.getElementById('excursionDetailPage');
  if (ep) { reviews.detachListeners(); ep.remove(); }
  nav.go(target || 'home');
}
...
const hotels = {
  render() {
    ...
    if (filtered.length === 0) { list.innerHTML = `<div class="text-center py-16"><div class="w-20 h-20 mx-auto mb-3 field-box rounded-full flex items-center justify-center"><i class="fa-solid fa-hotel text-2xl text-violet-300"></i></div><p class="font-display text-lg font-bold mb-1" style="color:var(--text-primary)">No hotels found</p><p class="text-sm" style="color:var(--text-secondary)">Try different filters</p></div>`; return; }
    ...
    return `
      <div onclick="showExcursionPage('${x.id}')" class="hotel-card rounded-[20px] overflow-hidden cursor-pointer">
        <div class="flex">
          <div class="relative w-32 h-32 md:w-36 md:h-36 flex-shrink-0 overflow-hidden">
            <img src="${imgSrc}" referrerpolicy="no-referrer" onerror="this.onerror=null; this.src='${PLACEHOLDER_IMG}'" class="w-full h-full object-cover">
            <div class="absolute bottom-2 right-2 rating-pill px-1.5 py-0.5 rounded-md flex items-center gap-1"><span class="text-[9px]" style="color:var(--text-secondary)">(${x.reviews} reviews)</span><i class="fa-solid fa-star text-gold-400 text-[8px]"></i><span class="text-[9px] font-bold text-gold-400">${Number(x.rating).toFixed(1)}</span></div>
          </div>
          <div class="flex-1 p-3 flex flex-col justify-between">
            <div>
...
function closeExcursionPage() { reviews.detachListeners(); const p = document.getElementById('excursionDetailPage'); if (p) p.remove(); nav.go('excursions'); }
...
function payAndConfirmExcursionBooking(subtotal, taxes, total) {
  ...
  kashier.pay({
    ...
    onSuccess: (result) => {
      const x = state.currentExcursion;
      const b = {
        id: orderId, type: 'excursion', excursionId: x.id, title: x.title, image: x.image, category: x.category,
        name: state.bookingDraft.name, email: state.bookingDraft.email, phone: state.bookingDraft.phone,
        date: state.bookingDraft.date, participants: state.bookingDraft.participants,
        payment: state.bookingDraft.payment, paymentStatus: result.status, transactionRef: result.transactionRef,
        priceUsd: total, priceFormatted: utils.formatPrice(total), status: 'upcoming', reviewed: false,
        createdAt: new Date().toISOString(), checkin: state.bookingDraft.date, uid: (fbEnabled && currentUser) ? currentUser.uid : null
      };
      persistBooking(b);
      renderBookingConfirmation(b);
    },
...
function payAndConfirmTransferBooking(subtotal, taxes, total) {
  ...
  kashier.pay({
    ...
    onSuccess: (result) => {
      const v = state.currentTransfer;
      const b = {
        id: orderId, type: 'transfer', transferId: v.id, vehicleType: v.vehicleType, image: v.image,
        direction: state.bookingDraft.direction, name: state.bookingDraft.name, email: state.bookingDraft.email, phone: state.bookingDraft.phone,
        flightNo: state.bookingDraft.flightNo, address: state.bookingDraft.address, date: state.bookingDraft.date, time: state.bookingDraft.time,
        passengers: state.bookingDraft.passengers, payment: state.bookingDraft.payment, paymentStatus: result.status, transactionRef: result.transactionRef,
        priceUsd: total, priceFormatted: utils.formatPrice(total), status: 'upcoming', reviewed: false,
        createdAt: new Date().toISOString(), checkin: state.bookingDraft.date, uid: (fbEnabled && currentUser) ? currentUser.uid : null
      };
      persistBooking(b);
...
const bookings = {
  switchTab(tab) {
    ...
    if (tab === 'upcoming') { up.className = 'flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-500 to-violet-700 text-white text-sm font-bold shadow-lg'; hist.className = 'flex-1 py-3 rounded-xl text-sm font-medium'; hist.style.color = 'var(--text-secondary)'; } else { hist.className = 'flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-500 to-violet-700 text-white text-sm font-bold shadow-lg'; up.className = 'flex-1 py-3 rounded-xl text-sm font-medium'; up.style.color = 'var(--text-secondary)'; }
    this.render();
  },
  render() {
...
const search = {
  ...
  filterDestination(dest) { nav.go('hotels'); utils.toast(`Showing ${dest} hotels`, 'info'); },
...
function applyHeroContext(tab) {
  ...
  setTimeout(() => {
    const eyebrowEntry = I18N_DICT[cfg.eyebrowKey], titleEntry = I18N_DICT[cfg.titleKey], subtitleEntry = I18N_DICT[cfg.subtitleKey];
    ...
    if (imgEl.src !== cfg.image) { imgEl.style.display = ''; imgEl.src = cfg.image; }
...
const auth = {
  ...
  async submit(e) {
    ...
    } catch (err) { utils.toast(err.message, 'error'); } finally { btn.textContent = original; btn.disabled = false; }
  },
  async googleSignIn() {
  ...
  async logout() {
    ...
    if (fbEnabled && fbAuth && currentUser) { await fbAuth.signOut(); } else { localStorage.clear(); location.reload(); }
  }
};
...
function arrayToKeyedObject(arr) { const o = {}; arr.forEach(item => { const copy = Object.assign({}, item); delete copy.id; o[item.id] = copy; }); return o; }
...
async function loadCatalogJson() {
  ...
  for (const f of files) {
    try {
      // ====== MODIFIED: قراءة من الـ Worker بدلاً من data/ ======
      const data = await API.readCatalogFile(f + '.json');
      CATALOG_RAW[f] = data;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  skeleton.fillAll();
  await loadAppConfig();
  CurrencyAPI.init().then(() => { refreshCatalogUI(); bookings.render(); });
  initFirebase();
});
