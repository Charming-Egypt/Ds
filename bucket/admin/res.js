...
class TripManager {
  ...
  async loadTripList(forceRefresh = false) {
    try {
      ...
      uiUtils.showToast(error.message || 'Failed to load trips', 'error');
    } finally {
      uiUtils.hideLoading();
    }
  }
  ...
  async loadTripForEditing(tripId, tripData) {
    try {
      ...
      uiUtils.showToast(error.message || 'Failed to load trip for editing', 'error');
  ...
  async saveTrip(e) {
    ...
    try {
      ...
      uiUtils.showToast(error.message || 'Failed to save trip', error.type || 'error');
    } finally {
      uiUtils.hideLoading();
  ...
  async deleteTrip(tripId) {
    ...
    try {
      ...
      uiUtils.showToast(error.message || 'Failed to delete trip', 'error');
    } finally {
      uiUtils.hideLoading();
    }
  }
