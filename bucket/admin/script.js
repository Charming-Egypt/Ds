...
// Set user profile data if logged in
const isUserLoggedIn = getCookie('username');

if (isUserLoggedIn) {
  // Set user info in form fields
  const userName = document.getElementById('userName');
  const userEmail = document.getElementById('userEmail');
  const userPhone = document.getElementById('userPhone');
  
  if (userName) userName.value = isUserLoggedIn;
  if (userEmail) userEmail.value = getCookie('email') || '';
  if (userPhone) userPhone.value = getCookie('phone') || '';
  ...
  const profilePhotos = document.querySelectorAll('.profile-photo, .profile-photo2');
  profilePhotos.forEach(photo => {
    photo.src = backgroundImageUrl || defaultPhoto;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  /**
   * Toggles bank fields visibility based on payout method
   */
  function toggleBankFields() {
    if (!payoutMethod || !bankFields) return;
