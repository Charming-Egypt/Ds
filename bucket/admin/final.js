...
const tripManager = {
  ...
  loadTripList: (forceRefresh = false) => {
    ...
    database.ref('trips').once('value').then(snapshot => {
      ...
      tripsArray = tripManager.filterAndSortTrips(tripsArray);
...
const setupEventListeners = () => {
  ...
  elements.searchInput.addEventListener('input', () => tripManager.loadTripList());
  elements.categoryFilter.addEventListener('change', () => tripManager.loadTripList());
  elements.sortFilter.addEventListener('change', () => tripManager.loadTripList());
};
