  ...
  function formatDate(dateStringOrDate) {
    ...
    return dateToFormat.toLocaleDateString(navigator.language || 'en-US', options);
  }
  ...
  async function resendVoucherEmailHandler() {
    ...
    if (!BookingData.email || !BookingData.email.includes('@')) {
      const email = prompt('Enter email to resend voucher:', BookingData.email || '');
  ...
  function handlePrintVoucher() {
    ...
    const originalInlineDisplay = voucherEl.style.display;
    voucherEl.style.display = 'block';
    printJS({
      printable: 'voucher-content',
      type: 'html',
      style: `.voucher-container { font-family: 'Segoe UI'; max-width: 800px; margin: auto; display: block !important; }`,
      scanStyles: false,
      onPrintDialogClose: () => voucherEl.style.display = originalInlineDisplay,
      onError: (err) => {
        voucherEl.style.display = originalInlineDisplay;
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      ...
      if (transactions.length > 0) {
        ...
        if (status === 'SUCCESS') {
          BookingData = { transactionId, cardBrand, maskedCard, totalPrice: amount, currency };
          populateVoucherDisplay(BookingData);
