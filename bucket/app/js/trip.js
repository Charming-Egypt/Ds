...
const TripDisplay = (() => {
  ...
  function formatPrice(p) {
    ...
    switch (currency) {
      case 'USD': return '$' + c.toFixed(2);
      case 'EUR': return '€' + c.toFixed(2);
      case 'GBP': return '£' + c.toFixed(2);
      default: return p.toFixed(2) + ' EGP';
    }
  }
  ...
  function initCurrency() {
    ...
    if (window.SharmCurrency?.rates) {
      rates = window.SharmCurrency.rates;
      ratesLoaded = true;
    }
    currencyHandler = e => {
  ...
  // SWIPER CONTROLS
  // ==========================================================================
  function hideSwiperCtrls() {
    const s = document.querySelector('.swiper');
    if (s) s.classList.add('swiper-controls-hidden');
  }

  function showSwiperCtrls() {
    const s = document.querySelector('.swiper');
    if (s) s.classList.remove('swiper-controls-hidden');
  }
  ...
  function playVideo(slide) {
    ...
    iframe.src = `https://www.youtube.com/embed/${vid}?autoplay=1&rel=0&modestbranding=1&controls=0&enablejsapi=1&playsinline=1`;
    ...
    ctrls.className = 'custom-video-controls';

    // Play/Pause
    const pp = document.createElement('button');
    pp.className = 'vc-btn vc-play';
    pp.innerHTML = '<i class="fas fa-pause"></i>';

    // Progress
    const pg = document.createElement('div');
    pg.className = 'vc-progress-wrap';
    const pf = document.createElement('div');
    pf.className = 'vc-progress-fill';
    pf.style.width = '0%';
    pg.appendChild(pf);

    // Time
    const tm = document.createElement('span');
    tm.className = 'vc-time';
    tm.textContent = '00:00 / 00:00';

    // Volume
    const vol = document.createElement('button');
    vol.className = 'vc-btn vc-volume';
    vol.innerHTML = '<i class="fas fa-volume-up"></i>';

    // Fullscreen
    const fs = document.createElement('button');
    fs.className = 'vc-btn vc-fullscreen';
    fs.innerHTML = '<i class="fas fa-expand"></i>';

    // Toggle Swiper Controls
    const tog = document.createElement('button');
    tog.className = 'vc-toggle-swiper';
    tog.innerHTML = '<i class="fas fa-eye"></i> Show Controls';

    // Close
    const cls = document.createElement('button');
    cls.className = 'vc-btn vc-close';
    cls.innerHTML = '<i class="fas fa-times"></i>';

    // Big play button
    const big = document.createElement('div');
    big.className = 'vc-big-play';
    big.innerHTML = '<i class="fas fa-pause"></i>';

    // Assemble
    ctrls.appendChild(pp);
    ctrls.appendChild(pg);
    ctrls.appendChild(tm);
    ctrls.appendChild(vol);
    ctrls.appendChild(fs);
    ctrls.appendChild(cls);
        ...
        events: {
          ...
          onStateChange: e => {
            if (e.data === 1) {
              ...
              big.innerHTML = '<i class="fas fa-play"></i>';
              pp.innerHTML = '<i class="fas fa-play"></i>';
              stopProg();
            } else if (e.data === 0) {
              playing = false;
              big.classList.remove('hidden');
              big.innerHTML = '<i class="fas fa-redo"></i>';
              pp.innerHTML = '<i class="fas fa-redo"></i>';
              stopProg();
            }
          }
        }
    ...
    function startProg() {
      stopProg();
      videoInterval = setInterval(updateProg, 300);
    }
    ...
    function updateProg() {
      ...
      try {
        ...
        if (d > 0) {
          pf.style.width = Math.min(100, (c / d) * 100) + '%';
          tm.textContent = fmtTime(c) + ' / ' + fmtTime(d);
        }
      } catch (e) {}
    }
    ...
    function showTemp() {
      ...
      tog.style.opacity = '1';
      big.classList.add('shift-up');
      clearTimeout(hideTimer);
      hideTimer = setTimeout(() => {
        if (playing) {
          ctrls.classList.remove('visible');
          tog.style.opacity = '0';
          big.classList.remove('shift-up');
        }
      }, 3000);
    }
    ...
    function onFullscreenChange() {
      ...
      fs.innerHTML = isFull ? '<i class="fas fa-compress"></i>' : '<i class="fas fa-expand"></i>';
    }
    ...
    pg.addEventListener('click', e => {
      ...
      player.seekTo(player.getDuration() * Math.max(0, Math.min(1, (e.clientX - r.left) / r.width)));
      updateProg();
    });
    ...
    vol.addEventListener('click', e => {
      ...
      vol.innerHTML = muted ? '<i class="fas fa-volume-mute"></i>' : '<i class="fas fa-volume-up"></i>';
    });
    ...
    tog.addEventListener('click', e => {
      ...
      if (!sw) return;
      const h = sw.classList.contains('swiper-controls-hidden');
      if (h) {
        showSwiperCtrls();
        tog.innerHTML = '<i class="fas fa-eye-slash"></i> Hide Controls';
      } else {
        hideSwiperCtrls();
        tog.innerHTML = '<i class="fas fa-eye"></i> Show Controls';
      }
  ...
  function createSlide(type, data, idx) {
    ...
    if (type === 'video') s.classList.add('swiper-slide-video');
    s.setAttribute('data-type', type);
    s.setAttribute('data-index', idx);

    if (type === 'video') {
      const vid = getYTId(data.videoUrl || data.url || '');
      if (!vid) return null;

      const thumb = getThumb(data, vid);
      s.setAttribute('data-video-id', vid);
      s.setAttribute('data-thumbnail', thumb);
      ...
      img.onerror = () => {
        ...
        s.setAttribute('data-thumbnail', img.src);
      };
      s.appendChild(img);
  ...
  function createThumbs() {
    const c = document.querySelector(S.thumbnailsOverlay);
    if (!c) return;
    c.innerHTML = '';
    ...
    document.querySelectorAll('.swiper-slide').forEach((s, i) => {
      ...
      const v = t === 'video';
      const src = v ? s.getAttribute('data-thumbnail') : s.querySelector('img')?.src;
      if (!src) return;

      const w = document.createElement('div');
      w.className = 'thumbnail-wrapper';
      w.setAttribute('data-index', i);
      w.setAttribute('data-type', t || 'image');
  ...
  function updateThumb(i) {
    document.querySelectorAll('.thumbnail-wrapper').forEach(w =>
      w.classList.toggle('active', parseInt(w.getAttribute('data-index')) === i)
    );
  }
  ...
  function initGallery(media) {
    ...
    const w = document.querySelector(S.swiperWrapper);
    if (!w) return;
    ...
    (media?.images || []).forEach(s => {
      if (s) {
        const sl = createSlide('image', s, c);
        if (sl) { w.appendChild(sl); c++; }
      }
    });

    (media?.videos || []).forEach(v => {
      if (v.videoUrl || v.url) {
        const sl = createSlide('video', v, c);
        if (sl) { w.appendChild(sl); c++; }
      }
    });
  ...
  function displayInfo(trip) {
    const t = document.querySelector(S.tourTitle);
    if (t) t.textContent = trip.name || '';

    const d = document.querySelector(S.tourDuration);
    if (d) d.textContent = trip.duration || 'Full Day';

    if (trip.name) document.title = trip.name + ' - Discover Sharm';

    const tn = document.querySelector(S.tripName);
    if (tn) tn.value = trip.name || '';
  }

  function displayDesc(desc) {
    const c = document.querySelector(S.descriptionContainer);
    const d = document.querySelector(S.tourDescription);
    if (!c) return;
    ...
    if (desc?.trim()) {
      d.innerHTML = desc
        ...
        .map(p => {
          const t = p.trim();
          if (t.startsWith('!') || t.startsWith('IMPORTANT:')) {
            return `<div class="highlight-note"><i class="fas fa-star" style="color:var(--gold);margin-right:8px;"></i>${sanitize(t.replace(/^!/, '').replace(/^IMPORTANT:/, '').trim())}</div>`;
          }
  ...
  function updatePrice() {
    const e = document.querySelector(S.tourPrice);
    if (e && currentTrip.basePrice) e.innerHTML = formatPrice(currentTrip.basePrice);
  }

  function loadIncluded(data) {
    const sections = [
      { c: S.includedItems, k: 'included', i: 'fa-check', cl: '#22c55e' },
      { c: S.notIncludedItems, k: 'notIncluded', i: 'fa-times', cl: '#ef4444' }
    ];

    sections.forEach(({ c, k, i, cl }) => {
      const el = document.querySelector(c);
      const items = data[k];
      if (el && items?.length) {
        el.innerHTML = items.map(it => `
          <div class="included-item">
            <i class="fas ${i}" style="color:${cl};"></i>
            <span>${sanitize(it)}</span>
          </div>
        `).join('');
      }
    });
  ...
  function loadTimeline(td) {
    const el = document.querySelector(S.timelineContainer);
    if (el && td?.length) {
      el.innerHTML = td.map(i => `
        <div class="timeline-item">
          <div class="timeline-time">${sanitize(i.time || '')}</div>
          <div class="timeline-content">
            <h4>${sanitize(i.title || '')}</h4>
  ...
  function loadBring(items) {
    const el = document.querySelector(S.whatToBringList);
    if (el && items?.length) {
      el.innerHTML = items.map(i => `<li><i class="fas fa-check"></i> ${sanitize(i)}</li>`).join('');
    }
  }
  ...
  function reviewSkeletons() {
    const c = document.querySelector(S.reviewsListContainer);
    if (c) {
  ...
  async function loadReviews() {
    ...
    try {
      ...
      tripReviews = data?.reviews
        ? Object.entries(data.reviews)
            .map(([id, r]) => ({ id, ...r }))
            .filter(r => r.approved)
            .sort((a, b) => new Date(b.date) - new Date(a.date))
        : [];

      updateStars(data?.average || 0, data?.count || 0);
      renderReviews();

      if (typeof auth !== 'undefined' && auth.currentUser) {
        currentUserReview = tripReviews.find(r => r.userId === auth.currentUser.uid);
        const btn = document.querySelector(S.openReviewBtn);
        if (btn) {
  ...
  function updateStars(avg, cnt) {
    const c = document.querySelector(S.avgStars);
    if (!c) return;
    ...
    c.innerHTML =
      ...
      '<i class="far fa-star"></i>'.repeat(5 - f - (h ? 1 : 0));

    const ce = document.querySelector(S.reviewsCountText);
    if (ce) ce.textContent = `(${cnt} ${cnt === 1 ? 'review' : 'reviews'})`;
  }
  ...
  function renderReviews() {
    ...
    if (!tripReviews.length) {
      c.innerHTML = '<div class="empty-state"><i class="fas fa-star"></i><p>No reviews yet</p><span>Be the first</span></div>';
      return;
    }

    c.innerHTML = tripReviews.map(r => {
      const d = new Date(r.date);
      const ds = isNaN(d) ? 'N/A' : `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
      const nm = r.userName || 'Traveler';
      const ini = nm.charAt(0).toUpperCase();
      const ph = getUserPhoto(r.userId);
      const st = Array(5).fill().map((_, i) => i < r.rating ? '<i class="fas fa-star"></i>' : '<i class="far fa-star"></i>').join('');
          ...
          <div class="review-card-header">
            <div class="review-card-user">
              <div class="review-card-avatar">
                <img src="${sanitize(ph)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" loading="lazy" alt="${sanitize(nm)}">
                <div class="avatar-fallback" style="display:none;">${sanitize(ini)}</div>
              </div>
              <div>
                <div class="review-card-user-name">${sanitize(nm)}</div>
                <div class="review-card-stars">${st}</div>
              </div>
            </div>
            <span class="review-card-date">${ds}</span>
          </div>
          <p class="review-card-comment">${sanitize(r.comment || '')}</p>
        </div>
      `;
    }).join('');
  }
  ...
  function openReviewModal() {
    ...
    const m = document.querySelector(S.reviewModal);
    if (m) {
  ...
  function setupStars() {
    const sc = document.querySelector(S.starSelector);
    if (!sc) return;

    sc.querySelectorAll('i').forEach(s =>
      s.addEventListener('click', () => {
        const r = parseInt(s.dataset.rating);
        document.querySelector(S.ratingValue).value = r;
        sc.querySelectorAll('i').forEach((st, i) => {
          st.classList.toggle('active', i < r);
          st.classList.toggle('fas', i < r);
          st.classList.toggle('far', i >= r);
        });
      })
    );

    const ci = document.querySelector(S.commentInput);
    const cc = document.querySelector(S.charCount);
    if (ci && cc) {
      ci.addEventListener('input', () => {
        let l = ci.value.length;
        if (l > 500) {
          ci.value = ci.value.substring(0, 500);
          l = 500;
        }
        cc.textContent = l;
      });
  ...
  async function submitReview() {
    ...
    const voucher = document.querySelector(S.voucherInput)?.value?.trim()?.toUpperCase();
    if (!voucher || !/^DS_[A-Z0-9]{8,}$/.test(voucher)) {
      toast('Invalid voucher', 'error');
      return;
    }

    const rating = parseInt(document.querySelector(S.ratingValue)?.value || '0');
    if (!rating || rating < 1) {
      toast('Select rating', 'error');
      return;
    }

    const comment = document.querySelector(S.commentInput)?.value?.trim();
    if (!comment || comment.length < 5) {
      toast('Min 5 characters', 'error');
      return;
    }

    try {
      const snap = await db.ref('trip-bookings/' + voucher).once('value');
      if (!snap.val() || snap.val().uid !== auth.currentUser.uid) {
        toast('Invalid voucher', 'error');
        return;
      }
      ...
      try {
        const us = await db.ref('egy_user/' + user.uid).once('value');
        if (us.val()) userName = us.val().username || 'Traveler';
      } catch (e) {}
      ...
      let total = (curr.average || 0) * count;
  ...
  function resetForm() {
    ...
    const rv = document.querySelector(S.ratingValue);
    if (rv) rv.value = '0';
    const cc = document.querySelector(S.charCount);
    if (cc) cc.textContent = '0';
    document.querySelectorAll(`${S.starSelector} i`).forEach(s => {
  ...
  // ==========================================================================
  function cleanup() {
    if (currencyHandler) window.removeEventListener('currencyChanged', currencyHandler);
    stopAllVideos();
    if (swiper) {
      swiper.destroy(true, true);
      swiper = null;
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (data && data[tripSlug]) {
        ...
        displayInfo(currentTrip);
        displayDesc(currentTrip.description);
        initGallery(currentTrip.media);
        loadIncluded(currentTrip);
        loadTimeline(currentTrip.timeline);
        loadBring(currentTrip.whatToBring);
        updatePrice();
      }
    } catch (e) {
  ...
  async function init() {
    ...
    if (typeof auth !== 'undefined') {
      auth.onAuthStateChanged(u => { currentUserUid = u?.uid || ''; });
    }
